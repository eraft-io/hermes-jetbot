...
class UartServoInfo:
	...
	def update(self, angle):
		...
		"""
		warnings.warn(
			"函数 update() 已弃用，请使用新函数 set_angle()",
			DeprecationWarning,  # 标准弃用警告
			stacklevel=2  # 让警告指向调用者，而不是函数内部
		)
		return self.set_angle(angle)	# 保持兼容
	

	def __str__(self):
		string = ""
		string += "|ID: " + str(self.id) + "\t"
		string += "|Angle: " + (str(self.angle) if self.angle is None else "{:.2f}°".format(self.angle)) + "\t"
		string += "|Power: " + (str(self.power) if self.power is None else "{:.2f}W".format(self.power/1000)) + "\t"
		string += "|Voltage: " + (str(self.voltage) if self.voltage is None else "{:.2f}V".format(self.voltage/1000)) + "\t"
		string += "|Current: " + (str(self.current) if self.current is None else "{:.2f}A".format(self.current/1000)) + "\t"
		string += "|Temp: " + (str(self.temp) if self.temp is None else "{:.2f}°C".format(self.temp)) + "\t"
		string += "|Turn: " + (str(self.turn) if self.turn is None else "{}".format(self.turn)) + "\t"
		string += "|Status: " + (str(self.status) if self.status is None else "{}".format(self.status)) + "\t"
		return string
...
class UartServoManager:
	...
	def query_servo_angle(self, servo_id, realtime=False):
		...
		is_response = self.update(wait_response=True) # 等待数据回传
		if realtime and not is_response:
			return None
	...
	def query_srv_info(self, servo_id, realtime=False):
		'''查询单个舵机的所有配置(未使用，无返回值)'''
	...
	def write_data(self, servo_id, address, content, realtime=False):
		...
		# 等待数据回传
		is_response = self.update(wait_response=True) 
		if realtime and not is_response:
			return False
	...
	def query_voltage(self, servo_id, realtime=False):
		'''查询总线电压,单位V'''
		voltage_bytes = self.read_data(servo_id, self.ADDRESS_VOLTAGE, realtime=realtime)
		if voltage_bytes is None:
			return None
	...
	def query_current(self, servo_id, realtime=False):
		'''查询电流, 单位A'''
		current_bytes = self.read_data(servo_id, self.ADDRESS_CURRENT, realtime=realtime)
	...
	def query_power(self, servo_id, realtime=False):
		'''查询功率, 单位W'''
		power_bytes = self.read_data(servo_id, self.ADDRESS_POWER, realtime=realtime)
		if power_bytes is None:
			return None
	...
	def query_temperature(self, servo_id, realtime=False):
		'''查询温度,为实际温度℃'''
		temp_bytes = self.read_data(servo_id, self.ADDRESS_TEMPERATURE, realtime=realtime)
		if temp_bytes is None:
			return None
		temp = float(1/((math.log(10000*(struct.unpack('<H', temp_bytes)[0])/(10000*(4096-(struct.unpack('<H', temp_bytes)[0]))))/3435)+1/(273.15+25))-273.15)
	...
	def query_status(self, servo_id, realtime=False):
		'''查询工作状态'''
		status_bytes = self.read_data(servo_id, self.ADDRESS_STATUS, realtime=realtime)
	...
	def send_sync_servo_monitor(self, servo_ids, realtime=False):
		...
		# 构建参数字节流
		param_bytes = struct.pack('<BBB', self.CODE_QUERY_SERVO_MONITOR, 1 ,servo_num)  # 包含命令编号和舵机ID数量
		for servo_id in servo_ids:
			param_bytes += struct.pack('<B', servo_id)  # 每个舵机ID单独打包
		# 发送请求
		self.send_request(self.CODE_SYNC_COMMAND, param_bytes)
		is_response = self.monitor_update(servo_ids)
		# print(is_response)
		if realtime:
			data = {}
			for servo_id in servo_ids:
				if is_response[servo_id]:
					data[servo_id] = self.servos.get(servo_id)
				else:
					data[servo_id] = UartServoInfo(servo_id)
			return data
		# 收集所有舵机的监控数据
	...
	def update(self, is_empty_buffer=False, wait_response=False, timeout=0.1):
		...
		# 相应回调数据
		# [self.pkt_buffer.update(struct.pack('<B', b)) for b in buffer]
		for b in buffer:
	...
	def monitor_update(self, servo_ids, timeout=0.1):
		'''舵机管理器的定时器回调函数'''
		servo_num = len(servo_ids) # 舵机ID数量
		t_start = time.time() # 获取开始时间
		t_start_p = time.time() # 获取开始时间
		buffer = bytearray()
		start_flag = 0
		ok_flag = 0
		bytes_waiting = 0
		loss_count = 0
		loss_flag = False
		is_response = {servo_id: False for servo_id in servo_ids}
		...
		while True:
			...
			if bytes_waiting:
				...
				if len(buffer) >=servo_num*21:
					break
				if (time.time() - t_start_p) > timeout:
					if len(buffer) < servo_num*21:
						loss_count +=1
					t_start_p = time.time()
					# print(f"{self.uart},miss {int((servo_num*21 - len(buffer))/21)} servo")
					# continue
				if time.time() - t_start > timeout*servo_num + timeout:
					# print("check",self.loss_uservo)
					if loss_count > servo_num:
						loss_flag = True
						break
			...
			if handler:
				...
				if loss_flag:
					...
					if angle == -235929599 and turn == 0:
						...
						is_response[servo_id] = True
			else:
				logging.warning('未知功能码 : {}'.format(code))	
		if not loss_flag:
			is_response = {servo_id: True for servo_id in servo_ids}
		# 清空原来的缓冲区
		self.pkt_buffer.empty_buffer()	
		return is_response
